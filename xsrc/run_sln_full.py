"""
Full SLN Experiment - Matches results_master.csv
==================================================

Runs SLN on ALL 520 baseline instances to match baseline results.

Usage:
    # Run everything (all 520 baseline instances)
    python run_sln_full.py
    
    # Run specific scenario
    python run_sln_full.py --scenario 4
    
    # Run specific scenario and APPEND to existing
    python run_sln_full.py --scenario 4 --append
    
    # Run specific size
    python run_sln_full.py --N 50
    
    # Test mode (first 5 instances)
    python run_sln_full.py --test 5
"""
...
def load_qubo_and_decompose(seed: int, scenario_id: int, N: int, data_dir: Path):
    """Load QUBO and decompose into components for SLN."""
...
def run_sln_on_instance(seed, scenario_id, N, data_dir, baseline_row, verbose=False):
    ...
    # Evaluate solution
    sln_eval = evaluate_solution(result['solution'], data)
    ...
    # Compute gaps
    gap_sa = sln_eval['raw_objective'] - sa_raw
    gap_greedy = sln_eval['raw_objective'] - greedy_raw
    gap_gurobi = sln_eval['raw_objective'] - gurobi_raw if not np.isnan(gurobi_raw) else np.nan
    
    # Win/tie/loss
    sln_beats_sa = int(gap_sa < -1e-6)
    sln_ties_sa = int(abs(gap_sa) <= 1e-6)
    sln_loses_sa = int(gap_sa > 1e-6)
    
    sln_beats_greedy = int(gap_greedy < -1e-6)
    sln_ties_greedy = int(abs(gap_greedy) <= 1e-6)
    
    if not np.isnan(gurobi_raw):
        sln_beats_gurobi = int(gap_gurobi < -1e-6)
        sln_ties_gurobi = int(abs(gap_gurobi) <= 1e-6)
        sln_loses_gurobi = int(gap_gurobi > 1e-6)
    else:
        sln_beats_gurobi = 0
        sln_ties_gurobi = 0
        sln_loses_gurobi = 0
    ...
    return {
        ...
        # SLN results
        'sln_raw_objective': sln_eval['raw_objective'],
        'sln_energy': result['energy'],
        'sln_feasible': int(sln_eval['feasible']),
        'sln_runtime_sec': result['runtime'],
        'sln_chosen_premium_band': sln_eval['chosen_premium_band'],
        'sln_chosen_deductible_band': sln_eval['chosen_deductible_band'],
        'sln_selected_features_count': sln_eval['selected_features_count'],
        'sln_selected_reg_features_count': sln_eval['selected_reg_features_count'],
        
        # SLN algorithm details
        'sln_lambda_0': result['lambda_0'],
        'sln_num_stages': len(result['stages']),
        'sln_best_stage': result['best_stage'],
        'sln_steps_used': result['steps_used'],
        'sln_final_condition_number': final_spec.get('condition_number', np.nan),
        'sln_final_negative_eigs': final_spec.get('negative_count', 0),
        'sln_final_roughness': final_spec.get('roughness', np.nan),
        ...
        # Gaps (negative = SLN better)
        'gap_sln_to_sa': gap_sa,
        'gap_sln_to_greedy': gap_greedy,
        'gap_sln_to_gurobi': gap_gurobi,
        
        # Performance indicators
        'sln_beats_sa': sln_beats_sa,
        'sln_ties_sa': sln_ties_sa,
        'sln_loses_sa': sln_loses_sa,
        'sln_beats_greedy': sln_beats_greedy,
        'sln_ties_greedy': sln_ties_greedy,
        'sln_beats_gurobi': sln_beats_gurobi,
        'sln_ties_gurobi': sln_ties_gurobi,
        'sln_loses_gurobi': sln_loses_gurobi,
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results/sln_full_results.csv')
    ...
    for idx, (scen_id, N, seed) in enumerate(tqdm(instances_to_run, desc="Running SLN"), 1):
    ...
    if results:
        ...
        # Overall stats
        print(f"\nSLN vs SA:")
        wins = df_new['sln_beats_sa'].sum()
        ties = df_new['sln_ties_sa'].sum()
        losses = df_new['sln_loses_sa'].sum()
        ...
        print(f"\nSLN vs Greedy:")
        wins_g = df_new['sln_beats_greedy'].sum()
        ...
        if df_new['gurobi_ran'].sum() > 0:
            ...
            matches = gurobi_df['sln_ties_gurobi'].sum()
            print(f"\nSLN vs Gurobi (where Gurobi ran):")
            print(f"  Matches: {matches:4d} / {len(gurobi_df)} ({matches/len(gurobi_df)*100:5.1f}%)")
        
        print(f"\nGap statistics (vs SA):")
        print(f"  Median: {df_new['gap_sln_to_sa'].median():>12,.1f}")
        print(f"  Mean:   {df_new['gap_sln_to_sa'].mean():>12,.1f}")
        ...
        if len(df_new['N'].unique()) > 1:
            ...
            for N_val in sorted(df_new['N'].unique()):
                ...
                w = df_n['sln_beats_sa'].sum()
                t = df_n['sln_ties_sa'].sum()
                gap = df_n['gap_sln_to_sa'].median()
