...
IMPROVED VERSION - Option A:
1. Stronger spectral smoothing (25% instead of 5%)
2. Wider beam search (20 instead of 6)
3. Better budget allocation (80% on final stage)
...
class SHOParameters:
    ...
    R_initial: int = 40  # Initial restarts (IMPROVED: was 20)
    B_beam: int = 20     # Beam width (IMPROVED: was 6)
    R_fresh: int = 0    # Fresh restarts per stage (e.g., 2)
    
    # Budget allocation (must sum to 1.0)
    stage_budgets: List[float] = None  # Fraction per stage, e.g., [0.05, 0.05, 0.1, 0.8]
...
def compute_lambda_0_gershgorin(Q: np.ndarray, epsilon_percentile: float = 0.25) -> float:
    """
    Compute initial spectral shift using Gershgorin lower bound.
    
    IMPROVED: Now uses 25% (was 5%) for much stronger smoothing.
    ...
    Args:
        ...
        epsilon_percentile: Safety margin as percentile of diagonal magnitudes
                           IMPROVED: default 0.25 (was 0.05)
...
def spectral_homotopy_QUBO(
    ...
    if verbose:
        print(f"\n{'='*70}")
        print(f"SPECTRAL HOMOTOPY OPTIMIZATION (IMPROVED)")
        print(f"{'='*70}")
    ...
    for stage_idx in range(1, len(params.t_stages)):
        ...
        # Beam tightening: Keep B=20 until stage 3, then tighten to B=10
        beam_size = 10 if stage_idx >= 3 else params.B_beam
...
# ============================================================================
# DEFAULT PARAMETERS - IMPROVED VERSION
# ============================================================================

def get_default_SHO_parameters() -> SHOParameters:
    """
    IMPROVED SHO parameters for better performance.
    
    Key improvements:
    1. Stronger smoothing: epsilon_percentile = 0.25 (was 0.05)
    2. Wider beam: B_beam = 20 (was 6)
    3. More initial diversity: R_initial = 40 (was 20)
    4. Better budget allocation: Focus on final stage [0.05, 0.05, 0.1, 0.8]
    5. Simplified to 4 stages (was 5) - more efficient
    
    Expected improvement: 2-3x better win rate
    """
    # IMPROVED budget allocation: Focus on final stage where solutions are found
    # 4 stages: t = [0.0, 0.25, 0.5, 1.0]
    stage_fractions = [0.05, 0.05, 0.1, 0.8]  # Was [0.4, 0.2, 0.15, 0.25]
    ...
    return SHOParameters(
        # Homotopy schedule (4 stages - simplified)
        t_stages=[0.0, 0.25, 0.5, 1.0],
        penalty_strategy="constant",
        
        # Constraint ramp parameters
        ...
        # IMPROVED: Beam parameters for better exploration
        R_initial=40,    # IMPROVED: Was 20 (more initial diversity)
        B_beam=20,       # IMPROVED: Was 6 (wider beam for exploration)
        R_fresh=0,       # Fresh restarts per stage
        
        # IMPROVED: Budget allocation focused on final stage
        stage_budgets=verified_fractions,  # [0.05, 0.05, 0.1, 0.8] vs [0.4, 0.2, 0.15, 0.25]
        total_budget=800000,
